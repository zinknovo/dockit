...
- cause_of_action: 案由
- hearing_time: 关键时间（如开庭、调解、谈话时间，格式：YYYY-MM-DD HH:mm）
- hearing_location: 关键地点（如法庭、调解室）
...
特别注意：
1. 即使是“谈话通知”或“调解通知”，也请将时间填入 hearing_time 字段。
2. 如果文书中有多个日期，优先提取“开庭”或“到场”日期。
字段不存在则 null。"""
...
def _classify_via_direct_llm(config: dict, document_text: str) -> dict:
    """无登录时直连 LLM（优先配置，其次环境变量）"""
    llm_cfg = config.get("llm") or {}
    api_key = llm_cfg.get("direct_api_key") or os.environ.get("OPENAI_API_KEY") or os.environ.get("LLM_API_KEY")
    
    if not api_key:
        raise ValueError("未配置 api_token，且无直连 API Key。请在设置中登录或配置 API。")
    
    from openai import OpenAI
    base_url = llm_cfg.get("direct_api_base") or os.environ.get("LLM_BASE_URL", "https://api.deepseek.com")
    model = llm_cfg.get("direct_model") or os.environ.get("LLM_MODEL", "deepseek-chat")
...
def classify(config: dict, document_text: str) -> DocumentInfo:
    ...
    if api_base_url and api_token:
        ...
        raw = _classify_via_direct_llm(config, document_text)

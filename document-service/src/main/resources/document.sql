...
CREATE TABLE IF NOT EXISTS `document` (
  ...
  `file_path` VARCHAR(512) COMMENT '历史遗留：git 仓库内相对路径（已弃用）',
  `scope_id` VARCHAR(64) COMMENT '历史遗留：git 仓库 scope（已弃用）',
...
CREATE TABLE IF NOT EXISTS `document_version` (
  ...
  `commit_hash` VARCHAR(64) COMMENT '历史遗留：git commit hash（已弃用）',
  `file_path` VARCHAR(512) COMMENT '历史遗留：git 仓库内相对路径（已弃用）',
  `note` TEXT COMMENT '用户备注',
  `uploaded_by` VARCHAR(64) COMMENT '上传人',
  `uploaded_at` DATETIME COMMENT '上传时间',
  `file_url` VARCHAR(1024) COMMENT 'MinIO 对象 key（原始文件存储位置）',
  `created_by` VARCHAR(64) COMMENT '创建人',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',

...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    { label: t('dashboard'), href: NAV_HREFS[0] },
    { label: t('policy'), href: NAV_HREFS[1] },
    { label: t('users'), href: NAV_HREFS[2] },
    { label: t('departments'), href: NAV_HREFS[3] },
    { label: t('userCenter'), href: NAV_HREFS[4] },
    { label: t('settings'), href: NAV_HREFS[5] }
  ]
  ...
  const activeHref = useMemo(() => {
    ...
    if (pathname.startsWith('/system/departments')) return '/system/departments'
  ...
  return (
    <div className="flex min-h-screen bg-[var(--default-bg-color)]">
      <aside className="hidden w-64 flex-col border-r border-[var(--default-border)] bg-[var(--default-box-color)] p-6 md:flex">
        <div className="mb-8">
          <p className="text-xs uppercase tracking-[0.2em] text-g-500">CompLog</p>
      ...
      <div className="flex flex-1 flex-col">
        <header className="flex h-16 items-center justify-between border-b border-[var(--default-border)] bg-[var(--default-box-color)] px-6">
